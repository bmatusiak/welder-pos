define(function(require, exports, module) {
    ...
    confirm("test","body",function(){
      ...
      */      
    function workorderTypeFNSetup(type,socket){
        var obj = {};
        if(type !== "workorder"){
    ...
    return function(docData) {
        ...
        window.socket(function (socket) {
            var actions = workorderTypeFNSetup(docData.type,socket);
            ...
            if(!loaded){
                socket.emit("workorder-load",docData.docid,function(err,doc){
            ...
            function autoSaveDoc(){
                ...
                socket.emit("workorder-calculate",getData(),function(err,calculatedData){
            ...
            function saveDoc(callback){
                if(docData.type == "workorder") return;
                if(!loaded || saving){ 
                    if(saving){
                        if(callback)
                ...
                socket.emit("workorder-save",docData.docid,draftObject,function(err,doc){
                    saving = false;
